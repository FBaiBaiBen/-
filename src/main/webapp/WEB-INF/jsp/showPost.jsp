...
<div class="card">
    <div class="card-header">
        <strong style="font-size: 18px;">职位信息列表</strong>
    </div>
    <div class="card-body px-0 pb-0">

        <div class="card-title">
            <form class="form-inline" id="select" action="${pageContext.request.contextPath}/post/showCondition"
                  method="post">
                <input type="hidden" name="page" value="1">
                <div class="form-group ml-4 mr-2">
                    <button class="btn btn-primary" type="button" data-toggle="modal" data-target="#Modal"
                            onclick="add(this)">添加职位
                    </button>
                </div>
                <div class="form-group mx-1">
                    <label>搜索：</label>
                    <input class="form-control" type="text" name="name" value="${post.name}" placeholder="请输入职位名称"/>
                </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <button type="button" class="btn btn-outline-primary" data-toggle="modal"
                                    data-target="#Modal" id="${post.id}" onclick="update(this)">修改
                            </button>
            ...
            <div class="modal-body">
                <div class="row justify-content-center">
                    <div class="col-md-8 order-md-1">
                        <form class="needs-validation" id="form" action="${pageContext.request.contextPath}/post/add"
...
<script type="text/javascript">
    function update(e) {
        $(".modal-title").text("添加职位");
        $("#form").attr("action", "${pageContext.request.contextPath}/post/add")
    }

    function update(e) {
        $(".modal-title").text("修改职位");
        var data = $(e).attr("id");
        $.post("${pageContext.request.contextPath}/post/toUpdate", {"id": data}, function (result) {
            $(".id").attr("value", result.id)
            $(".name").attr("value", result.name)
            $(".details").attr("value", result.details)
            $("#form").attr("action", "${pageContext.request.contextPath}/post/update")
        });
    }
